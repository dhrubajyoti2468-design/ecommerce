...
# ----------------------
# defaults (will not overwrite existing files)
# ----------------------
DEFAULT_USERS = [
...
DEFAULT_ORDERS = []
DEFAULT_PRODUCTS = []  # if you already have products.json content, it will load
...
def home():
    ...
    for p in products:
        ...
        if ok:
            pi = p.copy()
            r = [int(x) for x in p.get("ratings", []) if str(x).isdigit()]
            pi["avg_rating"] = (sum(r)/len(r)) if r else None
            filtered.append(pi)
...
def add_to_cart(pid):
    ...
    # If you want cart only for logged users, require login here (optional)
